...
const useGameHint = () => {
  const [hasSeen, setHasSeen] = useState<boolean>(
    localStorageUtils.hasSeenGameHints(),
  );
  const [hintIndex, setHintIndex] = useState<number>(
    hasSeen ? GAME_HINTS.length : 0,
  );
  const [hint, setHint] = useState<string | null>(
    hasSeen ? null : GAME_HINTS[hintIndex],
  );
  ...
  const nextHint = useCallback(() => {
    ...
    const newHint =
      hintIndex < GAME_HINTS.length - 1 ? GAME_HINTS[hintIndex + 1] : null;
