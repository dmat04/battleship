...
const messageListenerMiddleware: Middleware = (
  ...
  if (messageReceived.match(action)) {
    ...
    switch (code) {
      case ServerMessageCode.Error: break;
      case ServerMessageCode.OpponentMoveResult: break;
      case ServerMessageCode.OwnMoveResult: break;
      case ServerMessageCode.RoomStatusResponse: break;
      case ServerMessageCode.AuthenticatedResponse: break;
      case ServerMessageCode.GameStarted: {
        const { playerName } = state.gameRoom;
        const { playsFirst } = message;
        const notification = playsFirst === playerName
